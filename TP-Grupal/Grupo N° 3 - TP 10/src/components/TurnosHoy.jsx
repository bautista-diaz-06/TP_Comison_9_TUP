...
import React from "react";
import "../styles/Turno.css";
export default function TurnosHoy({ turnos }) {
  const hoy = new Date().toISOString().slice(0, 10);

  const turnosHoy = turnos.filter(t => t.fechaHora.startsWith(hoy));
    ...
    <div className="card">
      ...
      {turnosHoy.length === 0 ? (
        ...
        <ul>
          {turnosHoy.map(t => (
            <li key={t.id}>
              {new Date(t.fechaHora).toLocaleString()} • {t.clienteNombre} • {t.servicioNombre} • {t.estado}
            </li>
          ))}
        </ul>
      )}
